...
#ifdef ALGNAME
    fn_req = "keypairs_csharp_" STR(ALGNAME) ".rsp";
#else
    sprintf(fn_req, "keypairs_csharp_%d.rsp", CRYPTO_SECRETKEYBYTES);
#endif

#ifdef ALGNAME
    fn_rsp = "signed_csharp_ref_" STR(ALGNAME) ".rsp";
#else
    sprintf(fn_rsp, "signed_csharp_ref_%d.rsp", CRYPTO_SECRETKEYBYTES);
#endif
